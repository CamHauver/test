...
# Open and read csv
with open(cereal_csv, encoding="utf") as csv_file:
...
#Dictionaries are made up of keys and values 
    #cannot duplicate keys, will update with the latest key
    #key { }
    #index [ ]
...
my_info = {
        ...
        "age": 68,
        "hobbies": ["mountain biking", "bouldering", "violin"],
        "wake_up_times": {
            "Mondays": "0800",
            "Tuesdays": "0730",
            "Wednesdays": "0700",
...
print(f'{my_info["name"]} likes {my_info["hobbies"][0]} and wakes up at {my_info["wake_up_times"]["Wednesdays"]}' " on Wednesdays.")
...
fish = "halibut" #string is an array of letters ["h", "a", "l", ...]
...
# List comprehensions (looping inside the list) provide concise syntax for creating lists
...
def print_percentages(state_data):
    ...
    non_profit_students = int(state_data[3])
    for_profit_students = int(state_data[5])

    total_students = public_students + non_profit_students + for_profit_students
    
    # Find the total graduates
    public_graduates = int(state_data[2])
    non_profit_graduates = int(state_data[4])
    for_profit_graduates = int(state_data[6])

    total_graduates = public_graduates + non_profit_graduates + for_profit_graduates
    
    # Find the public school graduation rate

    public_school_graduation_rate = (public_graduates / public_students) * 100
    
    # Remember that some states do not have nonprofit or forprofit private schools
    # Find the non-profit school graduation rate
    if non_profit_students == 0:
        non_profit_graduation_rate == 0
    else:
        non_profit_graduation_rate = (non_profit_graduates / non_profit_students) *100
    
    # Find the for-profit school graduation rate
    if for_profit_students == 0:
        for_profit_graduation_rate == 0
    else:
        for_profit_graduation_rate = (for_profit_graduates / for_profit_students) *100
    ...
    print(f"{state}: ")
    print(f"public school graduation rate: {public_school_graduation_rate}")
    print(f"non profit graduation rate: {non_profit_graduation_rate}")
    print(f"for profit graduation rate: {for_profit_graduation_rate}")
    print(f"overall graduation rate: {overall_graduation_rate}")

    if (overall_graduation_rate > 50):
